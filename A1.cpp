...
void man(char * name)
{
	...
	{
		printf("Description : Used to close existing file\n");
...
void DisplayHelp()
{
	...
	printf("open  : to open thefile\n");
	printf("close : to Clear console\n");
	printf("closeall  : to close all opened thefile\n");
	...
	printf("fstat : to display the information of file using file File_Descriptor\n");
...
int LseekFile(int fd,int isize,int from)
{
	...
	if((UFDTArr[fd].ptrfiletable->mode == READ)||(UFDTArr[fd].ptrfiletable->mode == READ+WRITE))
	{
		if(from == CURRENT)
		{
			if(((UFDTArr[fd].ptrfiletable->readoffset)+isize)>UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize) return -1;
		...
		{
			if(isize > (UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize)) return -1;
			if(isize < 0 ) return -1;
		...
		else if(UFDTArr[fd].ptrfiletable->mode == WRITE)
		{
			if(from == CURRENT)
			{
				if(((UFDTArr[fd].ptrfiletable->writeoffset)+ isize)>MAXFILESIZE) return -1;
			...
			{	
				if((UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize)+isize > MAXFILESIZE) return -1;
				if(((UFDTArr[fd].ptrfiletable->writeoffset)+isize)< 0) return -1;
